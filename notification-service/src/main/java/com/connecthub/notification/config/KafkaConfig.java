        ...
        DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer(kafkaTemplate(),
                (consumerRecord, ex) -> {
                    log.error("DLQ: topic={} offset={} error={}",
                            consumerRecord.topic(), consumerRecord.offset(), ex.getMessage());
                    return new TopicPartition(consumerRecord.topic() + ".dlq", 0);
                });
        DefaultErrorHandler handler = new DefaultErrorHandler(recoverer, new FixedBackOff(2000L, 3L));
        handler.setRetryListeners((consumerRecord, ex, attempt) ->
                log.warn("Kafka retry {}/3 topic={}: {}", attempt, consumerRecord.topic(), ex.getMessage()));
        return handler;
    }
